"""Check if the Gmail OAuth refresh token is valid; re-authorize if it is not.

Usage:
    python google-contacts-integration/check_gmail_token.py
...
Actions secret GMAIL_REFRESH_TOKEN via the `gh` CLI.
...
def update_dotenv(new_token: str):
    """Write GMAIL_REFRESH_TOKEN into the .env file, creating it if absent."""
...
def update_github_secret(new_token: str):
    ...
    result = subprocess.run(
        ["gh", "secret", "set", "GMAIL_REFRESH_TOKEN", "--body", new_token, "--repo", repo],
        capture_output=True,
        text=True,
    )
    if result.returncode == 0:
        print(f"GitHub secret GMAIL_REFRESH_TOKEN updated for {repo}.")
    else:
...
def main():
    ...
    print("\nDone. Re-run the check_for_orders workflow to resume order checking.")
